...
interface RecipeData extends RecipeFormData {
  id: string;
  image?: string;
}
...
type EditSaveState = "idle" | "saving" | "saved" | "error" | "blocked";

const DEFAULT_INGREDIENT_ROWS = 3;
const EMPTY_CREATE_VALIDATION: CreateValidationState = {
  title: false,
  description: false,
  image: false,
...
function createInitialFormData(authorId = ""): RecipeFormData {
  return {
    title: "",
    description: "",
    tips: "",
...
function normalizeRecipeForm(recipe: RecipeData): RecipeFormData {
  return {
    title: recipe.title,
...
function getStepImageMap(steps: Step[] = []) {
...
function buildRecipeSteps(steps: Step[], stepImages: { [key: number]: string }) {
...
function getRecipeUpdateSignature(data: RecipeFormData, stepImages: { [key: number]: string }) {
  return JSON.stringify(buildRecipeUpdatePayload(data, stepImages));
}

function getEditValidationMessage(data: RecipeFormData) {
  if (!data.title.trim()) {
...
function getCreateValidationState(data: RecipeFormData, image: string | null): CreateValidationState {
  return {
...
function getCreateValidationLabels(validation: CreateValidationState) {
